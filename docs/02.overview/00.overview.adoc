...
The ADES is based on an Web API framework architecture. It exposes primarly a WPS Server with a legacy WPS 1.0 & 2.0 OWS services and it is offers a REST/JSON interface compliant with the OGC Processes API <<WPS-REST-JSON>> compliant with the OpenAPI specification <<OPENAPI>>.
