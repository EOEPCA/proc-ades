...
<<img_procAdesArchitecture>> shows the relationship between the WPS Server and the ADES core engine, and the interaction with the external Kubernetes cluster via the Argo Workflows API.

[#img_procAdesArchitecture,reftext='{figure-caption} {counter:figure-num}']
.ADES Architecture
image::processing-ADES-architecture.png[width=95%,pdfwidth=95%,align="center"]
