...
#ifndef PRODUCTION_CENTER_INIREADER_H
#define PRODUCTION_CENTER_INIREADER_H
...
public:
    ...
    std::vector<std::string> getKey(std::string part);
private:
    int _error;
...
inline std::vector<std::string> INIReader::getKey(std::string part){
    part=part+"=";
    std::vector<std::string> res;
    for(auto& u:_values){
        //std::strncmp( u.first.c_str(),part.c_str(),part.size());
        if(u.first.rfind(part,part.size())==0){
            try {
                res.push_back(u.first.substr(part.size(),u.first.size()-part.size()));
            }catch (...){
            }
        }
    }
...
#endif  //PRODUCTION_CENTER_INIREADER_H